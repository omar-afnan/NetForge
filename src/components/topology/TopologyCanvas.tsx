...
/**
 * Expand a list of routed (L3) hop devices into the full physical route:
 * consecutive hops that are not directly cabled (e.g. a PC and its router
 * on either side of a switch) are joined by the shortest path through the
 * actual rendered links, so the packet rides the real cables instead of
 * cutting a straight chord across intermediate devices.
 */
function expandOverLinks(
  hops: Device[],
  allDevices: Device[],
  ...
  function shortestPath(fromId: string, toId: string): Device[] | null {
    if (fromId === toId) return [deviceById.get(fromId)!]
    ...
    while (queue.length > 0) {
      ...
      for (const next of adjacency.get(current) ?? []) {
        ...
        if (next === toId) {
          ...
          while (walk) {
            chain.unshift(deviceById.get(walk)!)
...
export function TopologyCanvas() {
  ...
  useEffect(() => {
    ...
    const expanded = expandOverLinks(hopDevices, devices, links)
  ...
  }, [packetTrace, hostnameMap, devices, links, clearPacketTrace, size.w, size.h])
